/*
 * xloc - Advanced iOS Location Spoofer (Lite Edition)
 * 核心拦截苹果 /clls/wloc 接口回应，解包 ARPC，替换坐标/海拔后重新封包。
 * 已剥离冗余的地理位置解析与海拔请求功能，交由 CF Worker Web 端处理。
 */
(function () {
  "use strict";
  ...
  function normalizeConfig(input) {
    ...
    if (!Number.isFinite(cfg.rawLimit) || cfg.rawLimit < 0) cfg.rawLimit = 0;
  ...
  function serializeArpc(arpc) {
    return concatBytes([
      writeUInt16BE(arpc.version), writePascalString(arpc.locale), writePascalString(arpc.appIdentifier),
      writePascalString(arpc.osVersion), writeUInt32BE(arpc.functionId), writeUInt32BE(arpc.payload.length),
      arpc.payload
    ]);
  }
      ...
      if (lenOffset + 2 <= responseBytes.length) {
        ...
        if (realLen > 0 && realPayloadOffset + realLen <= responseBytes.length) {
          ...
          if (tryParseFields(candidatePayload) !== null) {
            return {
              kind: "marker", payload: candidatePayload, prefix: responseBytes.slice(0, markerIdx),
              markerAndLen: responseBytes.slice(markerIdx, realPayloadOffset), suffix: responseBytes.slice(realPayloadOffset + realLen)
            };
          }
        }
      }
  ...
  function spoofArpcRequest(requestBytes, configInput) {
    ...
    return { response: buildAppleWLocResponse(patched.payload), payload: patched.payload, wifiCount: patched.wifiCount, cellCount: patched.cellCount, arpc: arpc };
  }
  ...
  function spoofAppleResponse(responseBytes, configInput) {
    ...
    if (extraction.kind === "arpc") {
      var arpcOut = {
        version: extraction.arpc.version, locale: extraction.arpc.locale, appIdentifier: extraction.arpc.appIdentifier,
        osVersion: extraction.arpc.osVersion, functionId: extraction.arpc.functionId, payload: patched.payload
      };
    ...
    return { response: response, payload: patched.payload, wifiCount: patched.wifiCount, cellCount: patched.cellCount, kind: extraction.kind, prefix: extraction.prefix ? hexPreview(extraction.prefix, 8) : "" };
  }
  ...
  function parseArgumentString(argument) {
    ...
    var tailKeys = [
      "debug", "mode", "enabled", "latitude", "longitude", "altitude",
      "configHost", "configToken", "horizontalAccuracy", "verticalAccuracy",
      "unknownValue4", "motionActivityType", "motionActivityConfidence",
      "failOpen", "dumpRaw", "dumpHeaders", "prepareHeaders", "rawLimit"
    ];
    ...
    if (configUrlIdx >= 0) {
      ...
      var end = -1;
      for (var i = 0; i < tailKeys.length; i += 1) {
  ...
  function resolveConfigUrl(args) {
    ...
    if (host && token) return host + "/loc.json?token=" + encodeURIComponent(token);
    return "";
  }
  ...
  function enrichArgsFromPluginStore(args) {
    var keys = ["enabled", "latitude", "longitude", "altitude", "configHost", "configToken", "configUrl", "debug"];
    args = args || {};
    for (var i = 0; i < keys.length; i += 1) {
      var key = keys[i], current = args[key];
      if (current == null || current === "" || isPlaceholderValue(current)) {
        var stored = readPluginStoreArg(key);
        if (stored != null && !isPlaceholderValue(stored)) args[key] = stored;
      }
    }
    return args;
  }
  ...
  function detectRuntime() {
    if (typeof $environment !== "undefined" && $environment && $environment.product) return String($environment.product);
    if (typeof $loon !== "undefined") return "Loon";
    return "Unknown";
  }
  function isLoonRuntime() { return detectRuntime() === "Loon"; }
  ...
  function configFromArgs(args) {
    var cfg = {};
    var scalarKeys = [
      "enabled", "mode", "latitude", "longitude", "horizontalAccuracy", "verticalAccuracy",
      "altitude", "unknownValue4", "motionActivityType", "motionActivityConfidence",
      "failOpen", "debug", "dumpRaw", "dumpHeaders", "prepareHeaders", "rawLimit"
    ];
    if (args.config) cfg = mergeConfig(cfg, JSON.parse(args.config));
    if (args.configBase64) cfg = mergeConfig(cfg, JSON.parse(decodeBase64(args.configBase64)));
    for (var i = 0; i < scalarKeys.length; i += 1) {
      var key = scalarKeys[i];
      if (Object.prototype.hasOwnProperty.call(args, key)) cfg[key] = args[key];
    }
    return cfg;
  }

  function readRemoteConfigCache(url) {
    if (!url || typeof $persistentStore === "undefined" || !$persistentStore.read) return null;
    try {
      var raw = $persistentStore.read("location_spoofer_remote_cfg");
      if (!raw) return null;
      var entry = JSON.parse(raw);
      if (!entry || entry.url !== url || !entry.data) return null;
      if (Date.now() - entry.ts > 300000) return null; // 5 min expiry
      return entry.data;
    } catch (err) { return null; }
  }

  function writeRemoteConfigCache(url, data) {
    if (!url || typeof $persistentStore === "undefined" || !$persistentStore.write) return;
    try {
      $persistentStore.write("location_spoofer_remote_cfg", JSON.stringify({ url: url, data: data, ts: Date.now() }));
    } catch (err) {}
  }
  ...
  function fetchRemoteConfig(url, timeout, debug, callback) {
    ...
    $httpClient.get({ url: url, timeout: timeout || 3000 }, function (error, response, body) {
      if (error || !body) { callback(null, error || "empty body"); return; }
      try { callback(JSON.parse(body), null); } catch (err) { callback(null, err.message); }
    });
  }

  function refreshRemoteConfigCache(url, debug) {
    fetchRemoteConfig(url, 5000, debug, function (data, err) {
      if (data) writeRemoteConfigCache(url, data);
    });
  }
  ...
  function loadRuntimeConfigSync() {
    ...
    if (configUrl) {
      ...
      if (remoteCfg) cfg = mergeConfig(cfg, remoteCfg);
    }
  ...
  function loadRuntimeConfig(callback) {
    ...
    var cfg = loaded.cfg, configUrl = loaded.configUrl, debug = loaded.debug;

    function finish() {
      try { callback(normalizeConfig(cfg)); } catch (err) {
  ...
  function runMaintenanceCron() {
    ...
    if (configUrl) {
      ...
      fetchRemoteConfig(configUrl, 8000, debug, function (data, err) {
        if (data) writeRemoteConfigCache(configUrl, data);
        maybeDone();
      });
    }
  ...
  function doneSyntheticResponse(bytes, info) {
    ...
    if (isLoonRuntime()) {
      $done({ status: 200, headers: headers, body: bytes });
      return;
    }
    $done({ response: { status: 200, headers: headers, body: bytes } });
  }
  ...
  function continueResponseRewrite(config) {
    ...
    if (!responseBody || responseBody.length < 2) {
      donePassThrough(); return;
    }
    var responseResult = spoofAppleResponse(responseBody, config);
    doneRewriteResponse(responseResult.response, {
      wifiCount: responseResult.wifiCount, cellCount: responseResult.cellCount,
      debug: config.debug, targetLat: config.latitude, targetLng: config.longitude
    });
  }

  function prepareResponseBody(config) { prepareResponseBodySync(config); }
  ...
  function runShadowrocket() {
    ...
    if (!hasRequest && !hasResponse) {
      runMaintenanceCron(); return;
    }

    if (hasRequest && !hasResponse) {
      donePreparedRequestPassThrough(); return;
    }
    ...
    loadRuntimeConfig(function (config) {
      try {
        ...
        if (hasResponse) {
          if (config.mode === "probe" || config.mode === "inspect") { donePassThrough(); return; }
        ...
        if (config.mode !== "request") { donePassThrough(); return; }
        var requestBody = messageBodyToBytes($request);
        if (!requestBody || requestBody.length < 2) { donePassThrough(); return; }
        
        var requestResult = spoofArpcRequest(requestBody, config);
        doneSyntheticResponse(requestResult.response, {
          wifiCount: requestResult.wifiCount, cellCount: requestResult.cellCount, debug: config.debug
        });
      } catch (err) {
        if (config.failOpen !== false) { donePassThrough(); return; }
        $done({ response: { status: "HTTP/1.1 500 Internal Server Error", headers: { "Content-Type": "text/plain" }, body: "location spoofer failed: " + err.message } });
      }
    });
  }
